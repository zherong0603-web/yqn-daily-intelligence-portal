...
import { applyPublicRepoVariablesForLocalBuild, deliveryRoot as artifactDeliveryRoot, repoRoot, visualAuditRoot } from "./visualCommon.js";
...
async function buildManifest(commitHash: string): Promise<Record<string, unknown>> {
  ...
  return {
    ...
    delivery_version: "Daily Brief Refocus",
    commit_hash: commitHash,
    pages_url: pagesUrl,
    setup_url: `${pagesUrl}setup/`,
    about_url: `${pagesUrl}about/`,
    legacy_boss_url: `${pagesUrl}boss/`,
    legacy_executive_url: `${pagesUrl}executive/`,
...
<html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>打开 YQN 每日重点简报</title><meta http-equiv="refresh" content="0; url=./index.html"></head>
<body style="font-family: system-ui; background: #050812; color: #f2f7ff; padding: 32px;">
<h1>YQN 每日重点简报离线预览</h1>
...
<html lang="zh-CN"><head><meta charset="utf-8"><title>如何打开</title></head><body><h1>如何打开</h1><p>双击 open-here.html。离线预览不依赖密钥，不包含任何真实 secret。</p></body></html>`, "utf8");
  ...
  await cp(path.join(visualAuditRoot, "full-page"), path.join(packageRoot, "visual-audit", "full-page"), { recursive: true });
  await cp(path.join(visualAuditRoot, "sections"), path.join(packageRoot, "visual-audit", "sections"), { recursive: true });
  await cp(path.join(visualAuditRoot, "recordings"), path.join(packageRoot, "recordings"), { recursive: true });
...
2. \`visual-audit/full-page/desktop-report.png\`：单日简报页。
3. \`visual-audit/full-page/desktop-setup.png\`：自动化配置页。
4. \`visual-audit/full-page/desktop-about.png\`：系统说明页。
...
Demo 演示可以展示产品形态。真实日报要等 OPENAI_API_KEY 和 OPENAI_MODEL 配好、手动测试成功后再正式转发。
`;
  ...
  console.log(`[delivery] wrote ${zipPath}`);
}

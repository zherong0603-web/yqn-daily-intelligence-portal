...
function sourceFor(date: string, index: number, topic: Topic): BriefSource {
  const domains: Record<Topic, string> = {
    ai: "venturebeat.com",
    ecommerce_us_warehouse: "supplychaindive.com",
    xiaohongshu_b2b: "ad.xiaohongshu.com",
    personal_business: "indiehackers.com",
  };
  const domain = domains[topic];
  return {
    topic,
    name: `验收公开来源 ${index}`,
...
function sampleBrief(date: string, runId: string, encryptionEnabled: boolean): Brief {
  ...
  const items = sources.map((source, index) => itemFor(date, index + 1, source)).slice(0, 4);
  return briefSchema.parse({
    date,
    generated_at: new Date().toISOString(),
    one_liner: `验收样例日报已生成：${date} 可用于检查历史归档、搜索、主题筛选和分享打印。`,
    executive_summary: "这是一篇明确标记为验收样例的日报，用来验证 YQN Daily Intelligence Portal 的上线可用性。它不是商业事实简报，不替代真实每日情报；真实运行必须配置 OpenAI API Key 和 OPENAI_MODEL。",
    items,
    action_checklist: [
      "打开今日日报永久链接，确认老板可访问。",
      "搜索“美国仓”并使用主题筛选检查结果。",
      "点击复制分享链接，再测试打印或保存 PDF。",
    ],
    sources,
